...
class HiveServerClient:
  ...
  def call(self, fn, req, status=TStatusCode.SUCCESS_STATUS):
    ...
    if res.status.statusCode == TStatusCode.ERROR_STATUS and \
        re.search('Invalid SessionHandle|Invalid session|Client session expired|MD5: IO error acquiring password|Unable to instantiate org.apache.hadoop.hive.ql.metadata.SessionHiveMetaStoreClient', res.status.errorMessage or '', re.I):
